...
macro_data = pdr.get_data_fred(['GDPC1','CPIAUCSL','PCEC96','PCECC96','RSAFS','RRSFS','DSPIC96','RSAOMV','PSAVERT','PAYEMS','PRSCQ','B4701C0A222NBEA','CES0500000017','ICSA','CCSA','UMCSENT',
...
ticker='PCEC96'
#for ticker in regression_list:
gdp_data = macro_data.loc[:,'GDPC1'].dropna()
comp_data = macro_data.loc[:,ticker]
prediction_data = pd.merge_asof(left=gdp_data, right=comp_data, left_index=True, right_index=True, direction='backward')
prediction_data = prediction_data.dropna()

gdp_regression = sm.OLS(prediction_data.loc[:,'GDPC1'], prediction_data.loc[:,ticker])
